...
class Particle:
    ...
    def __init__(self, id, ff_code, bound_min, bound_max,
                 number_of_decimals):  # bound_min ,bound_max coulb be determined inside Particle but if it's done in Swarm it's faster
        ...
        self.x = self.generate_random_bitstring(self.bit_dim)  # x is BitArray
    ...
    def upgrade_vel(self, g_best):
        self.v = ((self.x ^ g_best) & self.generate_random_bitstring(self.bit_dim)) | (
                (self.x ^ self.p_best) & self.generate_random_bitstring(self.bit_dim))
        return
    ...
    def generate_random_bitstring(self, n):  # n should leave and be replaced with self.bit_dim in the function
        key1 = ""
        for i in range(n):
...
#     p.upgrade_pbest()
#     print(p)
#     p.upgrade_vel(p.generate_random_bitstring(p.bit_dim))
#     p.upgrade_position()
#
#
#     i+=1
